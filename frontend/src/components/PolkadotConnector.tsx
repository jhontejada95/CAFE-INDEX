...
const PolkadotConnector: React.FC<PolkadotConnectorProps> = ({
  onAccountChange,
}) => {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      if (availableAccounts.length === 0) {
        setConnectionError(
          "No se encontraron cuentas en la extensiu00f3n Polkadot.js",
        );
      } else {
  ...
  const handleSubmitTestPrice = async () => {
    ...
    try {
      ...
      const testPrice = 5.64; // precio de prueba
  ...
  return (
    <div className="card p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg transition-colors duration-300">
      <h3 className="text-xl font-semibold text-polkadot-pink-500 dark:text-polkadot-pink-300 mb-4">
      ...
      {extensionFound === false && (
        <div className="mb-4 p-3 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 rounded-lg transition-colors duration-300">
          <p>
            No se encontró la extensión Polkadot.js. Por favor instálala desde
            <a
      ...
      {selectedAccount ? (
        <div className="space-y-4">
            ...
            >
              {accounts.map((account) => (
                <option key={account.address} value={account.address}>
                  {account.meta.name} - {account.address.substring(0, 8)}...
          ...
          {submitResult && (
            <div
              className={`mt-3 p-3 rounded-lg transition-colors duration-300 ${
            ...
            >
              ...
              {!submitResult.success &&
                submitResult.message.includes("faucet.westend.network") && (
                  <div className="mt-2">
                    <a
